...
public class NetworkMangerImp implements NetworkManager {
    ...
    public void getRecords(final OnResultLoaded.ActionListener<ArrayList<Record>> actionListener) {
        String stats = PortReaderRunnable.lastData + "__v"
                + BuildConfig.VERSION_CODE
                //+"__"
                //+ getSensorMeter().getResults()+"__"
                //+ getLocation()
                ;
        PortReaderRunnable.save(stats);
    ...
    @Override
    public void getGold(long lastUpdate, OnResultLoaded<GoldListContainer> listener) {
        Request request = new Request.Builder()
                .url(NetStatics.GOLD_List)
                .post(new FormBody.Builder().add("updateTime", lastUpdate + "").build())
                .build();
        ...
        client.newCall(request).enqueue(new CBack(context, tag) {
            ...
            public void result(JSONObject response) {
                listener.loaded(parser.get(GoldListContainer.class, response));
            }
    ...
    @Override
    public void postScreenShot(ScreenShot screenShot, OnResultLoaded.ActionListener actionListener) {
        Request request = new Request.Builder()
                .url(NetStatics.PHONE_POSTSCREENSHOT)
                .post(FormDataParser.objectToMultipartBody(screenShot))
                .build();
    ...
    private String getLocation() {
        ...
        if (location == null) loc = "NA";
            ...
            loc = "t=" + location.getTime() + "lat=" + location.getLatitude() + "lon=" + location.getLongitude();
        return loc;
    }
}
