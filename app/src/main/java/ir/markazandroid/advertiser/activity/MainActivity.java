...
import ir.markazandroid.advertiser.hardware.PortReaderRunnable;
...
public class MainActivity extends BaseActivity implements SignalReceiver, VideoFragment.VideoStateChangeListener {
    ...
    private void showBars() {
        try {
            ...
            mSuDataOutputStream.writeBytes("am startservice --user 0 -n com.android.systemui");
            ...
            Log.e("console out","Out= "+builder.toString());*/
            //proc.waitFor();
    ...
    private void initMainLayout() {
        ...
        /*dateText.setOnClickListener( v -> hideBars());
    ...
    private void initJustGold(String title) {
        setContentView(R.layout.vertical_gold_activity_just_main);
        getSupportFragmentManager().beginTransaction().replace(R.id.goldFragmentContainer, GoldViewerFragment.getInstance(title)).commit();
        initMainLayout();
    }
    ...
    private void switchToGold(String title) {
        View advertiserContent = subtitle.getRootView().findViewById(R.id.advertiser_content);
        ((ViewGroup) advertiserContent.getParent()).removeView(advertiserContent);

        //View advertiser = getLayoutInflater().inflate(R.layout.vertical_gold_activity_main, (ViewGroup) subtitle.getRootView(),false);
        //((ViewGroup)advertiser.findViewById(R.id.advertiser_container)).addView(advertiserContent);

        setContentView(R.layout.vertical_gold_activity_main);
        ((ViewGroup) findViewById(R.id.advertiser_container)).addView(advertiserContent);
        getSupportFragmentManager().beginTransaction().replace(R.id.goldFragmentContainer, GoldViewerFragment.getInstance(title)).commit();
    ...
    private void startInit() {
        ...
        if (serialPort!=null){
            PortReaderRunnable portReader = new PortReaderRunnable(serialPort,this);
    ...
    private void initHardware() {
        //TODO
        portReader = new PortReaderRunnable(this);
        ThreadPoolExecutor threadPoolExecutor = (ThreadPoolExecutor) Executors.newCachedThreadPool();
        threadPoolExecutor.execute(portReader);
        //ttyS2
        ...
        if (record != null) {
            if (getRecorddownloader().isFinished()) {
                ...
                recordDownloader.setProcessMonitor(new RecordDownloader.ProcessMonitor() {
                    @Override
                    public void onProcess(final String status) {
                        runOnUiThread(() -> {
                            //  Log.e("status",status);
    ...
    private void initLayout(Record record) {
    ...
    private void initExtras(Record record) {
        ...
        initGoldExtras(record.getExtras());
    }

    private void initGoldExtras(ExtrasObject extras) {
        Signal signal = new Signal("Gold Title Loaded", Signal.GOLD_TITLE_LOADED, extras.getGoldTitle());
    ...
    private void init2PartExtras(ExtrasObject extrasObject) {
        if (extrasObject != null) {
            if (extrasObject.getWebViewUrl() != null && !extrasObject.getWebViewUrl().isEmpty()) {
                webView.loadUrl(fixUrl(extrasObject.getWebViewUrl()));
            }
        }
    }
